...
import com.mr208.rewired.common.handlers.EventHandlerClient;
...
/**
*This is an almost direct copy of Cyberware's RenderCyberlimbHand.
 * I needed to override a few private methods and this was easier
*/

public class RenderHandReWIRED
{
	...
	public static RenderHandReWIRED INSTANCE = new RenderHandReWIRED();
	
	public void renderItemInFirstPerson(AbstractClientPlayer abstractClientPlayer, float p_187457_2_, float p_187457_3_, EnumHand enumHand, float p_187457_5_, @Nullable ItemStack heldItemStack, float p_187457_7_)
	{
		boolean flag = enumHand == EnumHand.MAIN_HAND;
		...
		else if (heldItemStack.getItem() instanceof net.minecraft.item.ItemMap)
		{
			...
			{
				this.renderMapFirstPersonSide(p_187457_7_, enumhandside, p_187457_5_, heldItemStack);
			}
		}
		else
		{
			...
			if (abstractClientPlayer.isHandActive() && abstractClientPlayer.getItemInUseCount() > 0 && abstractClientPlayer.getActiveHand() == enumHand)
			{
				...
				switch (heldItemStack.getItemUseAction())
				{
					case NONE:
					...
					case EAT:
					case DRINK:
						this.transformEatFirstPerson(p_187457_2_, enumhandside, heldItemStack);
						this.transformSideFirstPerson(enumhandside, p_187457_7_);
						break;
					case BLOCK:
						this.transformSideFirstPerson(enumhandside, p_187457_7_);
					...
					case BOW:
						...
						float f5 = (float)heldItemStack.getMaxItemUseDuration() - ((float)this.mc.player.getItemInUseCount() - p_187457_2_ + 1.0F);
						float f6 = f5 / 20.0F;
			...
			this.renderItemSide(abstractClientPlayer, heldItemStack, flag1 ? ItemCameraTransforms.TransformType.FIRST_PERSON_RIGHT_HAND : ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND, !flag1);
		}
